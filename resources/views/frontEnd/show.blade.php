            ...
            <div class="row">
                <div class="col-lg-8" style="word-wrap: break-word; overflow-wrap: break-word;">
                    ...
                    <div class="alert alert-info">
                        {{ $mainPost->user->name ?? $mainPost->admin->name }}
                    </div>
...
@push('js')
    <script>
                ...
                success: function(data) {
                    ...
                    $.each(data, function(key, comment) {
                        $('.comments').append(`
                                ...
                                <div class="comment">
                                    <img src="/storage${comment.user.image }" alt="User Image" class="comment-img" />
                                    <div class="comment-content">
                                        <span class="username">${ comment.user.name }</span>
                                        <p class="comment-text">${ comment.comment }</p>
                                    </div>
        ...
        $(document).on('submit', '#commentForm', function(e) {
            ...
            // formData.append('_token', $('meta[name="csrf-token"]'); // add manually
            ...
            $.ajax({
                ...
                success: function(comment) {
                    ...
                    $('.comments').prepend(`
                                ...
                                <div class="comment">
                                    <img src="/storage${comment.data.user.image }" alt="User Image" class="comment-img" />
                                    <div class="comment-content">
                                        <span class="username">${ comment.data.user.name }</span>
                ...
                error: function(data) {
                    ...
                    $('#error-message').text(data.responseJSON.message).show();
                    setTimeout(function() {
                        $('#error-message').fadeOut();
                    }, 30000);
                }
